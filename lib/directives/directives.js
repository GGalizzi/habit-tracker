...
.directive('htProgress', function(Habits) {
  return {
    restrict: 'E',
    scope: {
      habit: '='
    ...
    link: function(scope, elem, attrs) {
      scope.$watch(function() { return Habits.values[scope.habit.name.toLowerCase()].level; }, function(val) {
        elem.css('width', val+"px");
        console.debug("things changed")
      });
    }
  };
});
